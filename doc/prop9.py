...
if 'recycle_points' not in st.session_state:
    # Define recycle points for each card
    st.session_state.recycle_points = {
        "death_guard": 1,
        "iron_warrior": 3,
        "ork": 5,
        "raven_guard": 7
...
if not st.session_state.card_history:
    ...
    history_data = {}
    total_recycle_points = 0
    
    # Count occurrences of each card name
    for entry in st.session_state.card_history:
        card_name = entry["card_name"]
        
        # Get recycle points for this card
        recycle_value = st.session_state.recycle_points.get(card_name, 0)
        
        if card_name not in history_data:
            history_data[card_name] = {
                "name": card_name,
                "count": 1,
                "recycle_points": recycle_value,
                "total_points": recycle_value
            }
            total_recycle_points += recycle_value
        else:
            history_data[card_name]["count"] += 1
            # Update total points for this card
            history_data[card_name]["total_points"] = history_data[card_name]["count"] * recycle_value
            total_recycle_points += recycle_value
    ...
    for card_name, data in history_data.items():
        table_data.append({
            ...
            "Recycle Points": data["recycle_points"],
    ...
    table_data.append({
        ...
        "Times Drawn": "",
        "Recycle Points": "",
        "Total Points": total_recycle_points
    })
...
# Sidebar with congratulatory message
with st.sidebar:
    st.header("Community Highlights")
    
    st.markdown("""
    ### 🎉 Congratulations Yinlong! 🎉
    
    Just drew the rare Raven Guard card  
    worth 7 recycle points!
    """)
    
    # Display the raven_guard image if available
    raven_guard_path = os.path.join(CARD_FRONTS_PATH, "raven_guard.png")
    if os.path.exists(raven_guard_path):
        raven_image = Image.open(raven_guard_path)
        st.image(raven_image, caption="Raven Guard - 7 points", width=150)
    else:
        st.info("Raven Guard image")
